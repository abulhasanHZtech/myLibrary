class DesignerTool{constructor(containerId){this.containerId=containerId;this.init()}loadDependencies(callback){const dependencies=[{type:"css",url:"https://fonts.googleapis.com/css2?family=Rubik:ital,wght@0,300..900;1,300..900&display=swap"},{type:"css",url:"https://unpkg.com/filepond/dist/filepond.min.css"},{type:"css",url:"https://cdn.quilljs.com/1.3.6/quill.snow.css"},{type:"css",url:"https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"},{type:"css",url:"https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css"},{type:"css",url:"https://cdnjs.cloudflare.com/ajax/libs/prism/1.23.0/themes/prism.min.css"},{type:"js",url:"https://cdnjs.cloudflare.com/ajax/libs/prism/1.23.0/prism.min.js"},{type:"js",url:"https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"},{type:"js",url:"https://code.jquery.com/jquery-3.5.1.slim.min.js"},{type:"js",url:"https://cdn.jsdelivr.net/npm/@popperjs/core@2.11.8/dist/umd/popper.min.js"},{type:"js",url:"https://cdn.quilljs.com/1.3.6/quill.js"},{type:"js",url:"https://cdn.rawgit.com/davidshimjs/qrcodejs/gh-pages/qrcode.min.js"},{type:"js",url:"https://cdnjs.cloudflare.com/ajax/libs/fabric.js/4.4.0/fabric.min.js"}];const loadScript=(src,callback)=>{const script=document.createElement("script");script.src=src;script.onload=callback;document.head.appendChild(script)};const loadStyle=(href,callback)=>{const link=document.createElement("link");link.rel="stylesheet";link.href=href;link.onload=callback;document.head.appendChild(link)};const loadNext=index=>{if(index>=dependencies.length){callback();return}const dependency=dependencies[index];if(dependency.type==="js"){loadScript(dependency.url,()=>loadNext(index+1))}else if(dependency.type==="css"){loadStyle(dependency.url,()=>loadNext(index+1))}};loadNext(0)}addCustomStyles(){const style=document.createElement("style");style.innerHTML=`
      body {
    ...
    </div>`;this.addCustomStyles();this.initializeTool()})}initializeTool(){let fabricCanvas=new fabric.Canvas("canvas",{selection:false});fabricCanvas.setHeight(600);fabricCanvas.setWidth(800);const upperCanvasElement=fabricCanvas.upperCanvasEl;upperCanvasElement.style.backgroundColor="transparent";fabricCanvas.renderAll();document.querySelectorAll(".size-btn").forEach(button=>{button.addEventListener("click",function(event){event.preventDefault();document.querySelectorAll(".size-btn").forEach(btn=>btn.classList.remove("active"));this.classList.add("active")})});document.querySelectorAll(".side-btn").forEach(button=>{button.addEventListener("click",function(event){event.preventDefault();document.querySelectorAll(".side-btn").forEach(btn=>btn.classList.remove("active"));this.classList.add("active")})});document.getElementById("canva_preview").addEventListener("click",function(){fabricCanvas.getObjects().forEach(function(object){object.selectable=false});fabricCanvas.discardActiveObject();fabricCanvas.renderAll();const sidebar=document.getElementById("sidebar");const mainSection=document.getElementById("main-section");sidebar.classList.remove("col-lg-4");sidebar.classList.add("col-lg-1");mainSection.classList.remove("col-lg-8");mainSection.classList.add("col-lg-11");document.getElementById("pills-tabContent").style.display="none";document.getElementById("pills-tab").style.display="none";document.getElementById("switch_btn").style.display="none";document.getElementById("back_tab").style.display="block";fabricCanvas.selection=false;fabricCanvas.off("mouse:down");fabricCanvas.off("mouse:up");fabricCanvas.off("mouse:move")});document.getElementById("back_tab_btn").addEventListener("click",function(){fabricCanvas.getObjects().forEach(function(object){object.selectable=true});const sidebar=document.getElementById("sidebar");const mainSection=document.getElementById("main-section");sidebar.classList.remove("col-lg-1");sidebar.classList.add("col-lg-4");mainSection.classList.remove("col-lg-11");mainSection.classList.add("col-lg-8");document.getElementById("pills-tabContent").style.display="block";document.getElementById("pills-tab").style.display="block";document.getElementById("switch_btn").style.display="block";document.getElementById("back_tab").style.display="none";fabricCanvas.selection=true;fabricCanvas.on("mouse:down");fabricCanvas.on("mouse:up");fabricCanvas.on("mouse:move")});document.querySelectorAll(".add_text_btn").forEach(button=>{button.addEventListener("click",function(){const text=this.getAttribute("data-text");let fontSize;switch(text){case"Heading 1":fontSize=36;break;case"Heading 2":fontSize=30;break;case"Heading 3":fontSize=24;break;case"Paragraph":fontSize=18;break;default:fontSize=20}const textbox=new fabric.Textbox(text,{left:10,top:20,width:200,fontSize:fontSize,fill:"black",selectable:true});fabricCanvas.add(textbox);fabricCanvas.renderAll();document.querySelector(".front_div").style.display="none";document.querySelector(".back_div").style.display="block"})});document.getElementById("fontFamily").addEventListener("change",function(){const activeObject=fabricCanvas.getActiveObject();if(activeObject&&activeObject.type==="textbox"){activeObject.set("fontFamily",this.value);fabricCanvas.renderAll()}});document.querySelectorAll(".text_weight button").forEach(button=>{button.addEventListener("click",function(){const activeObject=fabricCanvas.getActiveObject();if(activeObject&&activeObject.type==="textbox"){const buttonClass=this.className.split("_")[0];switch(buttonClass){case"bold":const isBold=activeObject.fontWeight==="bold";activeObject.set({fontWeight:isBold?"300":"bold"});this.classList.toggle("active",!isBold);break;case"italic":const isItalic=activeObject.fontStyle==="italic";activeObject.set({fontStyle:isItalic?"normal":"italic"});this.classList.toggle("active",!isItalic);break;case"underline":const isUnderline=activeObject.underline;activeObject.set({underline:!isUnderline});this.classList.toggle("active",!isUnderline);break}fabricCanvas.renderAll()}})});document.getElementById("fontWeight").addEventListener("change",function(){const activeObject=fabricCanvas.getActiveObject();if(activeObject&&activeObject.type==="textbox"){activeObject.set("fontWeight",this.value);fabricCanvas.renderAll()}});document.getElementById("fontSize").addEventListener("keyup",function(){const activeObject=fabricCanvas.getActiveObject();if(activeObject&&activeObject.type==="textbox"){const fontSize=this.value||20;activeObject.set({fontSize:parseInt(fontSize,10)});fabricCanvas.renderAll()}});document.getElementById("favcolor").addEventListener("change",function(){const activeObject=fabricCanvas.getActiveObject();if(activeObject&&activeObject.type==="textbox"){const color=this.value;activeObject.set({fill:color});fabricCanvas.renderAll()}});document.querySelectorAll(".text-align button").forEach(button=>{button.addEventListener("click",function(){document.querySelectorAll(".text-align button").forEach(btn=>btn.classList.remove("active"));this.classList.add("active");const alignment=this.className.split("_")[0];const activeObject=fabricCanvas.getActiveObject();if(activeObject&&activeObject.type==="textbox"){activeObject.set({textAlign:alignment});fabricCanvas.renderAll()}})});document.getElementById("back_btn").addEventListener("click",function(event){event.preventDefault();document.querySelector(".front_div").style.display="block";document.querySelector(".back_div").style.display="none"});let height;let width;let side;document.querySelector(".next_btn").addEventListener("click",function(event){event.preventDefault();const activeButton=document.querySelector(".size-btn.active");const sideButton=document.querySelector(".side-btn.active");let rect;function addRectangle(height,width){const canvasWidth=width;const canvasHeight=height;console.log("canvasWidth :",canvasWidth);console.log("canvasHeight :",canvasHeight);const rectWidth=346;const rectHeight=212;const rectLeft=canvasWidth-rectWidth;const rectTop=canvasHeight-rectHeight;rect=new fabric.Rect({top:rectTop,left:rectLeft,fill:"#ffffffad",angle:0,width:rectWidth,height:rectHeight,selectable:false,editable:false,lockMovementX:true,lockMovementY:true,lockScalingX:true,lockScalingY:true,lockRotation:true});fabricCanvas.add(rect);rect.bringToFront();fabricCanvas.renderAll();fabricCanvas.on("object:added",function(event){if(rect){rect.bringToFront();fabricCanvas.renderAll()}});fabricCanvas.on("selection:created",function(event){if(rect){rect.bringToFront();fabricCanvas.renderAll()}});fabricCanvas.on("selection:cleared",function(event){if(rect){rect.bringToFront();fabricCanvas.renderAll()}})}if(activeButton){height=activeButton.getAttribute("data-height");width=activeButton.getAttribute("data-width");side=sideButton.getAttribute("data-side");document.querySelector(".canvas_side").innerHTML=side;document.querySelector(".create_artwork_wrapper").style.display="none";const canvaView=document.querySelector(".canva-view");canvaView.style.display="flex";function adjustCanvasSize(){const canvasContainer=document.getElementById("canvasContainer");let divWidth=canvasContainer.offsetWidth;let divHeight=canvasContainer.offsetHeight;let canvasWidth=width*96;let canvasHeight=height*96;let scaleFactor=1;if(canvasWidth>divWidth||canvasHeight>divHeight){const widthScale=divWidth/canvasWidth;const heightScale=divHeight/canvasHeight;scaleFactor=Math.min(widthScale,heightScale);scaleFactor*=.88;console.log("Scale factor: ",scaleFactor)}fabricCanvas.setWidth(canvasWidth*scaleFactor);fabricCanvas.setHeight(canvasHeight*scaleFactor);fabricCanvas.setZoom(scaleFactor);if(side==="Back"){addRectangle(canvasHeight,canvasWidth)}fabricCanvas.renderAll()}adjustCanvasSize();window.addEventListener("resize",adjustCanvasSize)}else{console.log("No active button selected.")}});function shortenImageUrl(dataUrl,callback){const imageData=dataUrl.split(",")[1];fetch("https://postautopilot.com/version-test/api/1.1/obj/test",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer 9ab0cc4380d5f623e629a13016bc5603"},body:JSON.stringify({image:{filename:"image.jpg",contents:imageData}})}).then(response=>response.json()).then(data=>{const imageId=data.id;return fetch(`https://postautopilot.com/version-test/api/1.1/obj/test/${imageId}`,{method:"GET",headers:{Authorization:"Bearer 9ab0cc4380d5f623e629a13016bc5603"}})}).then(response=>response.json()).then(data=>{let shortUrl=data.response.image;if(shortUrl.startsWith("//")){shortUrl="https:"+shortUrl}callback(shortUrl)}).catch(error=>{console.error("Error:",error);alert("An error occurred while shortening the image URL.")})}document.getElementById("generateQR").addEventListener("click",function(event){event.preventDefault();const qrText=document.getElementById("qrText").value;const qr=new QRCode(document.createElement("div"),{text:qrText,width:128,height:128});const qrCanvas=qr._oDrawing._elCanvas;const qrDataUrl=qrCanvas.toDataURL("image/png");shortenImageUrl(qrDataUrl,function(shortUrl){fabric.Image.fromURL(shortUrl,function(img){img.set({left:50,top:50,selectable:true,isQR:true,scaledWidth:img.width,scaledHeight:img.height});fabricCanvas.add(img);img.bringToFront();fabricCanvas.renderAll();img.on("scaled",function(){img.set({scaledWidth:img.getScaledWidth(),scaledHeight:img.getScaledHeight()})});img.on("modified",function(){img.set({scaledWidth:img.getScaledWidth(),scaledHeight:img.getScaledHeight()})})})})});function handleTextSelection(activeObject){if(activeObject.type==="textbox"){console.log("here1",activeObject.fontWeight,activeObject.fontFamily);document.getElementById("fontSize").value=activeObject.fontSize;document.getElementById("fontFamily").value=activeObject.fontFamily||"Arial";const fontWeight=activeObject.fontWeight;if(fontWeight=="normal"){document.getElementById("fontWeight").value="300"}else{document.getElementById("fontWeight").value=fontWeight}const alignmentButtons=document.querySelectorAll(".text-align button");alignmentButtons.forEach(button=>button.classList.remove("active"));const alignment=activeObject.textAlign||"left";document.querySelector(`.text-align .${alignment}_align`).classList.add("active");document.querySelector(".text_weight .bold_btn").classList.toggle("active",activeObject.fontWeight==="bold");document.querySelector(".text_weight .italic_btn").classList.toggle("active",activeObject.fontStyle==="italic");document.querySelector(".text_weight .underline_btn").classList.toggle("active",!!activeObject.underline);document.getElementById("favcolor").value=activeObject.fill||"#000"}}document.querySelectorAll(".removeSelected").forEach(button=>{button.addEventListener("click",function(){const activeObject=fabricCanvas.getActiveObject();if(activeObject){fabricCanvas.remove(activeObject)}})});function handleSelectionEvent(e){const activeObject=e.target;console.log("activeObject",activeObject);if(activeObject){if(activeObject.type==="image"){if(activeObject.isQR){document.getElementById("pills-qrcode-tab").click()}else{document.getElementById("pills-image-tab").click()}}else if(activeObject.type==="textbox"){document.getElementById("pills-text-tab").click();document.querySelector(".front_div").style.display="none";document.querySelector(".back_div").style.display="block";handleTextSelection(activeObject)}}}fabricCanvas.on("selection:created",handleSelectionEvent);fabricCanvas.on("selection:updated",handleSelectionEvent);function addImageToCanvas(dataUrl,isQR=false){fabric.Image.fromURL(dataUrl,function(img){const canvasWidth=fabricCanvas.width;const canvasHeight=fabricCanvas.height;const imgWidth=img.width;const imgHeight=img.height;let scaleFactor=1;scaleFactor=Math.min(canvasWidth/imgWidth,canvasHeight/imgHeight);scaleFactor*=.88;const scaledWidth=imgWidth*scaleFactor;const scaledHeight=imgHeight*scaleFactor;img.set({left:0,top:0,scaleX:scaleFactor,scaleY:scaleFactor,selectable:true,scaledWidth:scaledWidth,scaledHeight:scaledHeight,isQR:isQR});fabricCanvas.add(img);img.bringToFront();fabricCanvas.renderAll();addImageToList(img);img.on("scaled",function(){img.set({scaledWidth:img.getScaledWidth(),scaledHeight:img.getScaledHeight()});fabricCanvas.renderAll()});img.on("modified",function(){img.set({scaledWidth:img.getScaledWidth(),scaledHeight:img.getScaledHeight()});fabricCanvas.renderAll()})},{crossOrigin:"anonymous"})}function addImageToList(img){const imageList=document.getElementById("imageList");const imageItem=document.createElement("div");imageItem.classList.add("image-item");imageItem.dataset.id=img.id;const imageElement=document.createElement("img");imageElement.src=img.getSrc();imageItem.appendChild(imageElement);const trashButton=document.createElement("button");trashButton.classList.add("trash-button");trashButton.innerHTML="&times;";trashButton.addEventListener("click",function(){fabricCanvas.remove(img);imageList.removeChild(imageItem)});imageItem.appendChild(trashButton);imageList.appendChild(imageItem)}document.getElementById("imageUploader").addEventListener("change",function(event){const file=event.target.files[0];if(!file){return}const reader=new FileReader;reader.onload=function(e){const dataUrl=e.target.result;shortenImageUrl(dataUrl,function(shortUrl){addImageToCanvas(shortUrl);console.log("shortUrl: ",shortUrl)})};reader.readAsDataURL(file)});document.getElementById("addImageFromUrl").addEventListener("click",function(event){event.preventDefault();const imageUrl=document.getElementById("imageUrl").value;if(!imageUrl){alert("Please enter a valid URL.");return}shortenImageUrl(imageUrl,function(shortUrl){addImageToCanvas(shortUrl)})});document.getElementById("flexSwitchCheckDefault").addEventListener("change",function(){const canvasContainer=document.getElementById("canvasContainer");const pillsTabContent=document.getElementById("pills-tabContent");const htmlEditor=document.getElementById("htmlEditor");const sidebar=document.getElementById("sidebar");const mainSection=document.getElementById("main-section");if(this.checked){canvasContainer.style.display="none";pillsTabContent.style.display="none";htmlEditor.style.display="block";sidebar.classList.remove("col-lg-4");sidebar.classList.add("col-lg-1");mainSection.classList.remove("col-lg-8");mainSection.classList.add("col-lg-11");var btn=document.getElementById("canva_preview");btn.disabled=true;const htmlContent=convertCanvasToHTML(fabricCanvas);const htmlCodeElement=document.getElementById("htmlCode");htmlCodeElement.textContent=htmlContent;Prism.highlightElement(htmlCodeElement)}else{canvasContainer.style.display="flex";pillsTabContent.style.display="block";htmlEditor.style.display="none";sidebar.classList.remove("col-lg-1");sidebar.classList.add("col-lg-4");mainSection.classList.remove("col-lg-11");mainSection.classList.add("col-lg-8");var btn=document.getElementById("canva_preview");btn.disabled=false}});function convertCanvasToHTML(canvas){let bodyContent="";let styles=[];canvas.getObjects().forEach((obj,index)=>{if(obj.type==="textbox"){bodyContent+=`<div class="textbox-${index}">
                  <p>${obj.text}</p>
                  </div>\n`;styles.push(`.textbox-${index} p { position: absolute; left: ${obj.left}px; top: ${obj.top}px; width: ${obj.width}px; font-size: ${obj.fontSize}px; font-weight:${obj.fontWeight}; color:${obj.fill}; font-family: ${obj.fontFamily}; }`)}else if(obj.type==="image"){bodyContent+=`<img class="image-${index}" src="${obj.getSrc()}">\n`;styles.push(`.image-${index} { position: absolute; left: ${obj.left}px; top: ${obj.top}px; width: ${obj.scaledWidth}px; height: ${obj.scaledHeight}px; }`)}else if(obj.type==="rect"){bodyContent+=`<div class="rect-${index}"></div>\n`;styles.push(`.rect-${index} { position: absolute; left: ${obj.left}px; top: ${obj.top}px; width: ${obj.width}px; height: ${obj.height}px; background-color: ${obj.fill}; }`)}});const fullHtml=`
  <!DOCTYPE html>
  <html lang="en">
  <head>
      <meta charset="UTF-8">
